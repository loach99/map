...
import { getPoints, setCheckboxFilter, setDataFilter, setLabels } from '../../store/filterReducer/actions/actions';
...
const ChartElement = () => {
    ...
    const { filtredDates, labels, checkboxFilter } = useSelector((state: RootState) => state.filter);
        ...
        <div>
            <div className={styles.chartHeader}>
                <div className={styles.chartDate}>
                    <div>
                        <input onChange={(e) => dispatch(setInputValue(e.target))} name='start_date' type="date" />
                    </div>
                    <div>
                        <input onChange={(e) => dispatch(setInputValue(e.target))} name='end_date' type="date" />
                    </div>
                </div>
            ...
            <Line data={data} height={width > 1200 ? 55 : width > 800 ? 80 : width > 600 ? 150 : 200} options={options} />
        </div>
    )
}
